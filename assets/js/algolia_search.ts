...
const algoliaHandler = () => {
  ...
  const isAlgoliaSettingsValid =
    ...
    algoliaSettings.apiKey; // 注意：这里不再检查 indexName，因为它是动态生成的
  ...
  // 1. 获取当前页面的语言设置
  const pageLang = document.documentElement.lang || 'en'; // 默认使用英文

  // 2. 根据语言动态选择索引名称
  let indexName;
  if (pageLang.toLowerCase() === 'zh-cn') {
    indexName = 'enkidu_site_zh-cn';
  } else {
    indexName = 'enkidu_site_en';
  }

  // --- 核心修改部分结束 ---

  const search = instantsearch({
    indexName: indexName, // 使用动态确定的索引名称
